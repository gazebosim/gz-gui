...
class ignition::gui::plugins::PalettePrivate
{
...
/////////////////////////////////////////////////
void Palette::LoadConfig(const tinyxml2::XMLElement */*_pluginElem*/)
{
  if (this->title.empty())
...
/////////////////////////////////////////////////
std::string Palette::HumanReadableKey(const std::string &_key)
{
    ...
    QStandardItem *_item, const std::string &_uri)
{
  ...
  for (int i = 0; i < count; ++i)
  {
    ...
    switch (field->type())
    {
      ...
      {
        auto humanName = this->HumanReadableKey(name);
      ...
      case google::protobuf::FieldDescriptor::TYPE_MESSAGE:
      {
        ...
        else if (field->message_type()->name() == "Quaternion")
        {
          ...
          for (auto it : rpy)
          {
            auto humanName = this->HumanReadableKey(it);
...
IGN_COMMON_REGISTER_SINGLE_PLUGIN(ignition::gui::plugins::Palette,
                                  ignition::gui::Plugin)
